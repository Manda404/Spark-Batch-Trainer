...
class XGBoostTrainer(BatchTrainer):
    ...
    def fit(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: ``model_config`` (XGBoost hyperparameters),
    ...
    def get_trained_model(self) -> Any:
        ...
        Returns:
            Any: Trained model object (e.g. ``XGBClassifier``).
        """
        return self._model
    ...
    def _fit_batch(
        ...
        """Fit the model on one batch, continuing from ``previous_model`` when available."""
        ...
        # eval_set order matters: XGBoost reports it back as validation_0/validation_1,
        # and sample_weight_eval_set must follow the same train-then-valid order.
        model.fit(
            batch_data["features"],
            batch_data["target"],
            eval_set=[
            ...
            verbose=runtime_state.get("verbose"),
    ...
    def _create_or_update_model(
        ...
        """Create a new XGBClassifier for the first batch, or update params for later ones."""
    ...
    def _evaluate_batch(
        ...
        # Most recent boosting iteration reflects this batch's end-of-training score.
    ...
    # Duplicated identically in LightGBMTrainer; kept per-backend rather than
    ...
    def _prepare_validation(
        ...
        """Collect, categorize, and weight the validation set; enables categorical support on ``model_config`` if needed."""
