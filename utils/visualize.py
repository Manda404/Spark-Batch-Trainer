...
def plot_class_distribution(
    dataset: pd.DataFrame, target_column: str, figsize: tuple[int, int] = (10, 5)
) -> pd.DataFrame:
    """
...
def _plot_confusion(
    ...
    cmn = confusion_matrix(y_true, y_pred, labels=labels, normalize="true") * 100
...
def plot_multiclass_confusion_matrices(
    ...
    matrices = (
        confusion_matrix(
            y_true_valid, y_pred_valid, labels=labels, normalize=normalization
        ),
        confusion_matrix(
            y_true_test, y_pred_test, labels=labels, normalize=normalization
        ),
    )
    ...
    figure.suptitle(
        "Multiclass Validation and Test Confusion Matrices",
        fontweight="bold",
        fontsize=14,
        y=1.02,
    )
